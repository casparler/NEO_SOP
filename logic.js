...
const SOP_LOGIC = {
    ...
    // Dosierung Fluconazol-Prophylaxe (IV) – off-label
    // Quelle (User-verifiziert, 06.05.2026):
    //  - VLBW <37 SSW & <1000 g  : 6 mg/kg/Dosis alle 3 Tage (72 h)
    //  - Neugeborene 0–14 d PNA  : 6 mg/kg alle 72 h (1 Dosis)
    //  - Neugeborene 14–28 d PNA : 6 mg/kg alle 48 h (1 Dosis)
    //  - Dauer: solange ein intravenöser Zugang vorhanden ist
    getFluconazol: function(ssw_dec, weight, pna) {
        const dosePerKg = 6;
        const isVLBW = (ssw_dec < 37.0) && (weight < 1.0);

        let interval;
        let regime;
        if (isVLBW) {
            interval = 72;
            regime = "VLBW (<37 SSW & <1000 g) – Standard q72h";
        } else if (pna <= 14) {
            interval = 72;
            regime = "Neugeborenes 0–14 Tage PNA – q72h";
        } else if (pna <= 28) {
            interval = 48;
            regime = "Neugeborenes 14–28 Tage PNA – q48h";
        } else {
            interval = null;
            regime = "PNA >28 Tage – Dosis-/Intervall-Schema [OFFEN, nicht in Quelle]";
        }

        return {
            dose: interval ? (weight * dosePerKg).toFixed(1) : null,
            interval: interval,
            unit: "mg",
            regime: regime,
            note: "off-label · IV · solange i.v.-Zugang"
        };
    },
